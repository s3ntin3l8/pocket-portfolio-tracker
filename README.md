...
| `docker-compose.prod.yml`       | `pocket-portfolio-tracker` | The deployed stack (`api` + `web`), pull-only from GHCR. Always needs `-f docker-compose.prod.yml --env-file .env.prod` — use `make prod`/`prod-down`/`prod-logs`/`prod-ps` instead of typing that out. `make prod-build` (composes with `docker-compose.prod.build.yml`) is the escape hatch for testing an unmerged change on the host. |
| `docker-compose.prod.build.yml` | `pocket-portfolio-tracker` | Override of `docker-compose.prod.yml` that re-adds local `build:` blocks for both services. Used only by `make prod-build`; `make prod` ignores it.                                                                                                                                                                                       |
