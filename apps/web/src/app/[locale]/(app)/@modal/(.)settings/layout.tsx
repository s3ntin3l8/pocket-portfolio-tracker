...
/**
 * The intercepted (`(.)settings`) counterpart to `app/[locale]/(app)/settings/layout.tsx`
 * — same nav items, same `SettingsShell` rail/landing, only reached via in-app client
 * navigation (a direct load/refresh of `/settings` always hits the real layout instead;
 * Next's route interception is soft-navigation-only). The one real difference is the
 * chrome it wraps `SettingsShell` in: `SettingsModalShell`'s centered-Dialog/full-screen
 * overlay instead of being embedded in the normal page flow — see issue #588.
 *
 * Deliberately duplicates the real layout's nav-item/identity-card construction rather
 * than extracting a shared helper — a small, easily-diffed duplication felt lower-risk
 * here than a novel shared-data abstraction touching the already-shipped `/settings/*`
 * layout under this PR's time budget. Keep the two in sync by hand if `Settings.nav*`
 * copy or the section list ever changes.
 */
