...
export function BenchmarkCard({
  ...
  return (
    <Card className="rounded-[20px] bg-card p-4 shadow-card">
      <p className="text-xs font-semibold text-text-2">vs {benchmark.symbol}</p>
      ...
      <p className="mt-1 text-xs font-medium text-text-2">Active return</p>
      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-text-2">
        <span>
          Tracking error: <span className="font-semibold">{trackingError > 0 ? `${(trackingError * 100).toFixed(1)}%` : "—"}</span>
        </span>
        <span>
          Correlation: <span className="font-semibold">{correlation ? correlation.toFixed(2) : "—"}</span>
        </span>
      </div>
    </Card>
  );
