...
export function VolatilityCard({
  ...
  return (
    <Card className="rounded-[20px] bg-card p-4 shadow-card">
      <p className="text-xs font-semibold text-text-2">Volatility & Risk</p>
      {vol !== null ? (
      ...
      <p className="mt-1 text-xs font-medium text-text-2">Annualized volatility</p>
      <div className="mt-2 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-text-2">
        {sharpe !== null && (
          <span>
            Sharpe: <span className="font-semibold">{sharpe.toFixed(2)}</span>
          </span>
        )}
        {sortino !== null && (
          <span>
            Sortino: <span className="font-semibold">{sortino.toFixed(2)}</span>
          </span>
        )}
      </div>
    </Card>
